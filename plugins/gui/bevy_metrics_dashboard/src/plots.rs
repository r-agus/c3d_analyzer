...
pub struct GaugeData {
    source: Arc<AtomicU64>,
    smoother: Smoother,
    principal_ring: Ring<f64>,          // Principal ring is used to store the data for the entire duration of the program
    secondary_ring: Option<Ring<f64>>,  // If Real Time is enabled, secondary_ring is used to store the data till the current frame
...
impl GaugeData {
    fn new(config: GaugePlotConfig, source: Arc<AtomicU64>) -> Self {
        ...
        Self {
            ...
            principal_ring: Ring::new(window_size),
            secondary_ring: Some(Ring::new(0)),
            config,
        }
    }
    ...
    fn handle_global_config(&mut self, dash_config: &DashboardConfig) {
        if let Some(window_size) = dash_config.global_window_size {
            self.config.window_size = window_size;
            self.principal_ring.set_max_len(self.config.window_size);
        }
    }
    ...
    fn update_real_time(&mut self) {
        ...
        self.principal_ring.push(self.smoother.smoothed_value());
    }

    fn update_static_time(&mut self, all_values: Vec<f64>, frame: usize) {
        if self.secondary_ring.is_none() {
            self.secondary_ring = Some(Ring::new(frame));
        }
        self.principal_ring.fill(all_values.clone());
        if self.secondary_ring.is_some() {
            self.secondary_ring.as_mut().unwrap().fill(all_values[0..frame].to_vec());
        }
    }
}
...
impl HistogramData {
    ...
    fn update(&mut self) {   // If Real Time is disabled, update function creates new green plot for data till the current frame
...
impl MetricPlot {
    ...
    pub fn clone_config(&self) -> MetricPlotConfig {
        match &self.metric_type {
            MetricPlotType::Counter(data) => MetricPlotConfig::Counter(data.config.clone()),
            MetricPlotType::Gauge(data) => MetricPlotConfig::Gauge(data.config.clone()),
            MetricPlotType::Histogram(data) => MetricPlotConfig::Histogram(data.config.clone()),
        }
    }
    ...
    pub fn update(&mut self, frame: usize) {
        self.frame = frame;
        match &mut self.metric_type {
            MetricPlotType::Counter(data) => {
                data.update();
            }
            MetricPlotType::Gauge(data) => {
                if self.real_time {
                    data.update_real_time();
                } else {
                    data.update_static_time(self.values.clone(), self.frame);
                }
            }
            MetricPlotType::Histogram(data) => {
                data.update();
            }
        }
    }
...
fn draw_plot(
    ...
    name: &str,
    unit: Option<Unit>,
    data: &mut MetricPlotType, // HistogramData, CounterData, GaugeData
    ui: &mut Ui,
) {
    let new_plot = || {
        Plot::new(name)
    ...
    match data {
        ...
        MetricPlotType::Gauge(data) => {
            if let Some(latest) = data.principal_ring.latest() {
                ui.label(format!("latest = {latest:.3}"));
            }

            let mut plot_points = data.principal_ring.make_plot_points();
            if data.config.derivative {
                derivative(&mut plot_points);
            }
            let principal_line = Line::new(PlotPoints::Owned(plot_points));
            let secondary_line = data
                .secondary_ring
                .as_ref()
                .map(|ring| {
                    let mut plot_points = ring.make_plot_points();
                    if data.config.derivative {
                        derivative(&mut plot_points);
                    }
                    Line::new(PlotPoints::Owned(plot_points)).color(Color32::GREEN)
                });
